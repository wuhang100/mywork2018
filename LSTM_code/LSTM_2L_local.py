...
#print result
row1 = int(round(0.2 * result.shape[0]))
row2 = int(round(0.2 * result.shape[0]))
row3 = int(round(0.3 * result.shape[0]))
train = result[:row1, :]
...
model.fit(
    X_train, y_train,
    batch_size=100, epochs=60,validation_split=0.2,verbose=0)
...
plot_model(model, to_file='/home/wuhang/thesispaper/modelLSTM2.png', show_shapes=True, show_layer_names=True)
